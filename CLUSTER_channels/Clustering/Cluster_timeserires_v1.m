...
close all;
sub=2;
Ev12=[];Ev34=[];%FFna5=zeros(5,106);FFca5=zeros(5,106);% for 5 channels (5, 106)
%%for all features in a row vector (1, 5*106=530)
FFna5=zeros(1,530);FFca5=zeros(1,530);
for j=1:sub
EN=4;
for i=1:EN
...
% cluster-1 and so on
 ...
 %xlswrite(ffc,FFc);
 %figure
%   t=linspace(-10,200,length(FFc));
%   figure(1);
%   plot(t,FFc'*1e6,'LineWidth',2);
...
%   xlim([-10 200]); grid on;xlabel('Time (ms)');ylabel('Voltage (uv)')
 ...
 cs1m=mean(cs1); cs2m=mean(cs2); cs3m=mean(cs3);cs4m=mean(cs4);cs5m=cs5;
  ...
  FFnr34=[cs1m;cs2m;cs3m;cs4m;cs5m];% this for 5 by106
  FFn34=[cs1m cs2m cs3m cs4m cs5m]; % this is for 1 by 530
  filename=['c15_' 'S' num2str(j) 'event34.xlsx'];
  ffn=fullfile(pathw,filename); % file name with folder location
  FFca5=FFca5+FFc12;
  FFna5=FFna5+FFn34;
  %xlswrite(ffn,FFn34)
  %Ev12=[Ev12;FFcr12];
