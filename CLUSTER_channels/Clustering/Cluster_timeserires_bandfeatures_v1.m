...
close all;
sub=2;
Fs=500; % sampling frequence
Ev12=[];Ev34=[];FFna5=zeros(5,106);FFca5=zeros(5,106); clfes=[];nofes=[];
for j=1:sub
EN=4;
for i=1:EN
...
%   xlim([-10 200]); grid on;xlabel('Time (ms)');ylabel('Voltage (uv)')
 ...
 cs1m=mean(cs1); cs2m=mean(cs2); cs3m=mean(cs3);cs4m=mean(cs4);cs5m=cs5;
  ...
  Ev34=[Ev34;FFn34];
  %%
  [tf af bf gm]=EEG_feat(FFcr12,Fs);
  alcleanfe =[tf af bf gm]; % Features stack by row for clear sound
  clfes=[clfes;alcleanfe];
  %%nose-degraded
  [tfn afn bfn gmn]=EEG_feat(FFnr34,Fs);
  alnofes =[tfn afn bfn gmn]; % Features stack by row for clear sound
  nofes=[nofes;alnofes];
end
%xlswrite('clearbandfeatures.xlsx',clfes); %% This the file for clear band features writing
%xlswrite('noisebandfeatures.xlsx',nofes); %% This is the for nose-degraded band features writing
toc;
%%% Average anb ploting
%{
Fallcc5=FFca5/sub;% 
